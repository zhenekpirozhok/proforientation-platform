...
test('guest can complete quiz via API (bulk + submit)', async ({
  request,
  baseURL,
}) => {
  ...
  const verRes = await request.get(
    `${baseURL}/api/quizzes/${quizId}/versions/current`,
    {
  ...
  expect(
    verRes.ok(),
    await dump(verRes, 'current version failed'),
  ).toBeTruthy();
  ...
  expect(
    startRes.ok(),
    await dump(startRes, 'start attempt failed'),
  ).toBeTruthy();
  ...
  while (true) {
    const qRes = await request.get(
      `${baseURL}/api/questions/quiz/${quizId}?page=${page}&size=${size}`,
      {
        headers: { 'x-locale': locale },
      },
    );
    ...
    expect(
      qRes.ok(),
      await dump(qRes, `questions page=${page} failed`),
    ).toBeTruthy();
  ...
  const bulkRes = await request.post(
    `${baseURL}/api/attempts/${attemptId}/answers/bulk`,
    {
      headers: {
        'x-locale': locale,
  ...
  const submitRes = await request.post(
    `${baseURL}/api/attempts/${attemptId}/submit`,
    {
      headers: {
        'x-locale': locale,
        'x-guest-token': guestToken,
