...
async function safeClick(locator: Locator, attempts = 12) {
  let lastErr: unknown;
...
async function safeClickViaEvaluate(locator: Locator) {
  await locator.waitFor({ state: 'visible', timeout: 15000 });
  await locator.scrollIntoViewIfNeeded();
  await locator.evaluate((el: HTMLElement) => el.click());
}

test('quiz flow: catalog -> details -> play -> submit -> results', async ({
  page,
  baseURL,
}) => {
