...
export function useGetQuizVersions(id?: number): UseQueryResult<QuizVersionDto[] | undefined, unknown> {
    const enabled = typeof id === 'number' && Number.isFinite(id) && id > 0;
    return generatedUseGetVersions(enabled ? id : 0, { query: { enabled } }) as any;
}
