...
type StoreState = ReturnType<typeof useQuizPlayerStore.getState>;
type ResultType = StoreState['result'];
...
describe('quiz-player store', () => {
  ...
  describe('startFresh', () => {
    test('sets starting state and clears attempt/answers/result', () => {
      useQuizPlayerStore.setState({
        ...
        result: { attemptId: 777 } as unknown as ResultType,
  ...
  describe('resumeOrStart', () => {
    ...
    test('does not resume if completed (result present)', () => {
      useQuizPlayerStore.setState({
        ...
        currentIndex: 1,
        totalQuestions: 3,
        result: { attemptId: 555 } as unknown as ResultType,
      });
  ...
  describe('result / bulk', () => {
    test('setResult and setBulkSent update state', () => {
      getState().setResult({ attemptId: 1 } as unknown as ResultType);
  ...
  describe('resetAll', () => {
    test('resetAll returns to initial state', () => {
      useQuizPlayerStore.setState({
        ...
        totalQuestions: 10,
        answersByQuestionId: { 1: 2 },
        result: { attemptId: 1 } as unknown as ResultType,
        bulkSentAttemptId: 1,
      });
  ...
  describe('persist / rehydrate', () => {
    test('rehydrate loads persisted fields but forces status=idle and error=null', async () => {
      ...
      await (useQuizPlayerStore as unknown as PersistApi).persist.rehydrate();
