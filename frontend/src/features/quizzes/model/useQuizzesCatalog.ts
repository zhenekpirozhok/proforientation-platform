...
import type { QuizDto, ProfessionCategoryDto, Search1Params } from '@/shared/api/generated/model';
...
function durationToRange(duration: string): { minDurationSec?: number; maxDurationSec?: number } {
  const shortMax = 15 * 60;
  const midMax = 35 * 60;

  if (duration === 'short') return { maxDurationSec: shortMax };
  if (duration === 'mid') return { minDurationSec: shortMax + 1, maxDurationSec: midMax };
...
export function useQuizzesCatalog(params: {
  ...
  const shouldSearch = hasServerSearch || hasServerCategory || hasServerDuration;
  ...
  const searchParams = useMemo<Search1Params | undefined>(() => {
    ...
    if (hasServerDuration) {
      ...
      if (typeof r.minDurationSec === 'number') p.minDurationSec = r.minDurationSec;
      if (typeof r.maxDurationSec === 'number') p.maxDurationSec = r.maxDurationSec;
    }
  ...
  const itemsAll = useMemo<QuizCatalogItem[]>(() => {
    ...
    const quizzes =
      rawSearch && !shouldSearch
        ? base.filter((x) => (x.title ?? '').toLowerCase().includes(rawSearch.toLowerCase()))
        : base;
    ...
    return quizzes.map((q) => {
      ...
      const category = metric?.categoryId != null ? categoriesById.get(metric.categoryId) : undefined;
