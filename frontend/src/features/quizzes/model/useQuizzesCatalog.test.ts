...
const useDebounceMock = jest.fn(<T>(v: T): T => v);

jest.mock(
  '@/entities/quiz/api/useQuizzes',
  () => ({
    useQuizzes: (p: unknown) => useQuizzesMock(p),
  }),
  { virtual: true },
);

jest.mock(
  '@/entities/quiz/api/useSearchQuizzes',
  () => ({
    useSearchQuizzesLocalized: (l: unknown, p: unknown) =>
...
jest.mock(
  ...
  () => ({
    useGetAllMetrics: (p: unknown) => useGetAllMetricsMock(p),
  }),
  { virtual: true },
);

jest.mock(
  '@/entities/category/api/useCategories',
  () => ({
    useCategories: (l: unknown) => useCategoriesMock(l),
  }),
  { virtual: true },
);
...
jest.mock(
  ...
  () => ({
    useDebounce: <T>(v: T) => useDebounceMock(v),
  }),
  { virtual: true },
);

function makeQuery<T = unknown>(
...
describe('useQuizzesCatalog', () => {
  ...
  test('joins quizzes with metrics and categories', () => {
    ...
    expect(out.items.map((x: { id: number }) => x.id)).toEqual([1, 2]);
  });
  ...
  test('local search filters when length < 2', () => {
    ...
    expect(out.items.map((x: { id: number }) => x.id)).toEqual([1]);
  });
  ...
  test('uses search API when search length >= 2', () => {
    ...
    expect(out.items.map((x: { id: number }) => x.id)).toEqual([3]);
  });
  ...
  test('isLoading and errors are propagated', () => {
    ...
    useCategoriesMock.mockReturnValueOnce(makeQuery({ error: e3 }));
