...
export function validateScales(scales: ScaleDraft[]): ValidationErrors {
  ...
  for (const s of scales) {
    ...
    if (!s.description.trim())
      add(out, `scale.${s.tempId}.description`, 'required');
  }
  ...
  if (bipolar.length > 0) {
    ...
    for (const [pairId, arr] of groups.entries()) {
      ...
      const codes = new Set(
        arr.map((x) => (x.bipolarPairCode ?? '').trim()).filter(Boolean),
      );
    ...
    for (const o of q.options) {
      ...
      if (neededTraitIds.length > 0) {
        ...
        for (const tid of neededTraitIds) {
          if (!(tid in map))
            add(out, `o.${o.tempId}.weights`, 'weightsMissing');
        }
      }
    }
  }
...
export function validateResults(results: ResultsDraft) {
  ...
  if (
    ...
    results.selectedCategoryIds.length === 0
  ) {
