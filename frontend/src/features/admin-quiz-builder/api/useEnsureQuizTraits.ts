...
function isBipolarPair(s: ScaleDraft) {
    return s.polarity === 'bipolar' && !!s.pairId && (s.side === 'LEFT' || s.side === 'RIGHT');
}
...
export function useEnsureQuizTraits(actions: ReturnTypeUseQuizBuilderActions | null) {
    ...
    return useCallback(async () => {
        ...
        try {
            ...
            for (const s of single) {
                ...
                const traitId = (res as any).id as number | undefined;
            ...
            for (const g of bipolarGroups) {
                ...
                const pairCode = (left.bipolarPairCode || right.bipolarPairCode || '').trim();
                ...
                const resL = await actions.createTrait.mutateAsync({ data: payloadLeft });
                const resR = await actions.createTrait.mutateAsync({ data: payloadRight });

                const leftId = (resL as any).id as number | undefined;
                const rightId = (resR as any).id as number | undefined;

                if (typeof leftId === 'number') patchScale(left.tempId, { traitId: leftId });
                if (typeof rightId === 'number') patchScale(right.tempId, { traitId: rightId });
            }

            message.success(t('traitsCreated'));
            return true;
        } catch (e) {
