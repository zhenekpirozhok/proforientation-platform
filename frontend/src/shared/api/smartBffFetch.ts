...
/**
 * Auth-first, fallback to guest (PUBLIC endpoints)
 */
export async function smartBffFetch(
  ...
  const authed = await bffAuthFetch(upstreamPath, { ...init, headers });
  ...
  return bffFetch(upstreamPath, { ...init, headers: guestHeaders });
}

/**
 * STRICT auth (ADMIN endpoints) — no guest fallback
 */
export async function smartBffFetchStrict(
  req: NextRequest,
  upstreamPath: string,
  init: SmartFetchInit,
) {
  ...
  return bffAuthFetch(upstreamPath, { ...init, headers });
}
