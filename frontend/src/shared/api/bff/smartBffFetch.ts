...
function pickGuestToken(req: NextRequest) {
    return req.headers.get('x-guest-token') ?? req.headers.get('guest-token') ?? undefined;
}

export async function smartBffFetch(req: NextRequest, upstreamPath: string, init: SmartFetchInit) {
