...
const AVG_SECONDS_PER_QUESTION = Number(process.env.QUIZ_AVG_SECONDS_PER_QUESTION ?? 8);
...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
    ...
    if (!quizRes.ok) {
        return new NextResponse(quizBody || JSON.stringify({ message: "Failed to load quiz" }), {
    ...
    const qRes = await bffFetch(`/questions/quiz/${quizId}?page=0&size=1`, { method: "GET" });
    ...
    if (qRes.ok) {
        ...
        if (Array.isArray(qJson)) {
            ...
            if (typeof qJson.totalElements === "number") questionCount = qJson.totalElements;
            else if (Array.isArray(qJson.content)) questionCount = qJson.content.length;
        }
    }
    ...
    if (typeof questionCount !== "number") {
        ...
        for (let i = 0; i < 200; i++) {
            const chunkRes = await bffFetch(`/questions/quiz/${quizId}?page=${page}&size=${size}`, {
                method: "GET",
            });
    ...
    const estimatedSeconds = Math.max(0, Math.round(questionCount * AVG_SECONDS_PER_QUESTION));
