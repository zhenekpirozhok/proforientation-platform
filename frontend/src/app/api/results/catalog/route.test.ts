...
function upstreamErr(status: number, body: string) {
    return {
        ok: false,
        status,
        async text() {
...
describe('api/results/catalog GET', () => {
    beforeEach(() => {
        ...
        parseResponseMock.mockImplementation((res: any) => res.__data);
    });

    test('returns 400 when quizId is missing/invalid', async () => {
        const r1 = await GET(makeReq('http://localhost/api/results/catalog'));
        expect(r1.status).toBe(400);
        expect(await r1.text()).toBe(JSON.stringify({ message: 'quizId is required' }));

        const r2 = await GET(makeReq('http://localhost/api/results/catalog?quizId=abc'));
        expect(r2.status).toBe(400);

        const r3 = await GET(makeReq('http://localhost/api/results/catalog?quizId=-1'));
    ...
    test('returns 502 when quiz categoryId is missing', async () => {
        ...
        const res = await GET(makeReq('http://localhost/api/results/catalog?quizId=12'));
    ...
    test('happy path joins quiz/traits/professions and filters by categoryId; forwards x-locale', async () => {
        ...
        bffFetchMock.mockImplementation(async (path: string, init?: any) => {
        ...
        expect(body.professions.map((p: any) => p.id).sort()).toEqual([1, 3]);
        expect(body.professions.every((p: any) => p.categoryId === 10)).toBe(true);
    ...
    test('returns 500 when upstream returns non-ok (traits)', async () => {
        ...
        bffFetchMock.mockImplementation(async (path: string) => {
            if (path === `/quizzes/${quizId}`) return upstreamOk({ id: quizId, categoryId: 1 });
            if (path === '/traits') return upstreamErr(503, 'bad');
            throw new Error(`unexpected ${path}`);
        });

        const res = await GET(makeReq(`http://localhost/api/results/catalog?quizId=${quizId}`));
