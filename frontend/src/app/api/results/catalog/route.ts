...
async function fetchAllProfessions(headers?: Record<string, string>) {
    ...
    for (let guard = 0; guard < 200; guard++) {
        ...
        const res = await bffFetch(`/professions?${sp.toString()}`, { method: "GET", headers });
...
export async function GET(req: Request) {
    try {
        ...
        // 1) quiz -> categoryId
        const quizRes = await bffFetch(`/quizzes/${quizId}`, { method: "GET", headers });
        const quiz = await fetchJsonOrThrow<QuizDto>(quizRes, `/quizzes/${quizId}`);

        const categoryId = quiz.categoryId;
        if (!Number.isFinite(categoryId)) {
            return new Response(JSON.stringify({ message: "Quiz categoryId is missing", quiz }), {
                status: 502,
                headers: { "content-type": "application/json" },
            });
        }
