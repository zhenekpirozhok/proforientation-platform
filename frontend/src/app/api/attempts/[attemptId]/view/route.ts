...
import type {
  AttemptViewDto,
  AttemptViewTrait,
  AttemptViewProfession,
} from '@/entities/attempt/model/types';
...
export async function GET(req: Request) {
  ...
  const [resultRes, traitsRes, professionsRes] = await Promise.all([
    ...
    bffFetch('/professions?page=1&size=200&sortBy=id', {
      method: 'GET',
      headers,
    }),
  ]);
  ...
  if (!traitsRes.ok)
    return new Response(await traitsRes.text(), {
      status: traitsRes.status,
      headers: traitsRes.headers,
    });
  if (!professionsRes.ok)
    return new Response(await professionsRes.text(), {
      status: professionsRes.status,
      headers: professionsRes.headers,
    });

  const result = await parseResponse<AttemptResultDto>(resultRes);
  const traits = await parseResponse<TraitDto[]>(traitsRes);

  const professionsData = await parseResponse<
    PageLike<ProfessionDto> | ProfessionDto[]
  >(professionsRes);
  ...
  const traitScores = Array.isArray(result.traitScores)
    ? result.traitScores
    : [];
